...
function setProxy(options, config, location) {
  ...
  if (proxy) {
    ...
    if (isHttps.test(options.protocol)) {
      ...
      // Workaround: avoid ERR_TLS_CERT_ALTNAME_INVALID when connecting to proxy
      // with custom cert that only has 127.0.0.1 as its CN/SAT
      // (https://github.com/TooTallNate/node-https-proxy-agent/issues/131)
      if (proxyAgentOptions.servername === undefined) {
        proxyAgentOptions.servername = proxy.hostname || proxy.host;
      }
  ...
  options.beforeRedirect = function beforeRedirect(redirectOptions) {
    // Make sure a previous https-proxy-agent isn't used, e.g. if no proxy should be used
    // for an HTTPS request or if using an HTTPS proxy for an HTTP connection.
...
module.exports = function httpAdapter(config) {
  return new Promise(function dispatchHttpRequest(resolvePromise, rejectPromise) {
    ...
    options.agent = isHttpsRequest ? options.httpsAgent : options.httpAgent;
    if (config.transport) {
