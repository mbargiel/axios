...
function setProxy(options, config, location) {
  ...
  const optionsBeforeRedirect = options.beforeRedirect;
  options.beforeRedirect = function beforeRedirect(redirectOptions) {
    // Reset request options to original config values in case a different proxy config is used
    ...
    // Chain any user-specified beforeRedirect
    optionsBeforeRedirect && optionsBeforeRedirect(redirectOptions);
  };
}
